...
percolate_datadir = path.expanduser(f"~/scratch/MAC_percolate/sample-{nsample}/")
...
all_temps = np.arange(80,510,5,dtype=np.float64)
...
for k, T in enumerate(temps):
    ...
    for n in range(nloops):
        print(k)
        ts[k] += hopsys.MCpercolate_dipoles(Js,T)/nloops
    print('\n\n')

np.save(ts, f'dipole_perc_times-{nsample}-{rank}.npy')