...
gate_names = ['$\epsilon_0$', '$\epsilon_{\\text{F}}$', '$\epsilon_{N-100}$']
...
for st, rmax, lbl, c in zip(structypes,r_maxs,ensemble_lbls,clrs):
    print(f'\n---------- {st} ----------')
    for x, mt in zip(range(1,4),motypes):
        ...
        if x == 1:
            ax.errorbar(x,sigma,yerr=sig_err,fmt='o',label=lbl,ms=30.0,lw=15.0,c=c)
        else:
            ax.errorbar(x,sigma,yerr=sig_err,fmt='o',ms=30.0,lw=15.0,c=c)
